...
class SafetySupervisor
{
  private:
    ...
    grid_map::GridMap gridmap_;  //({"StaticObjects", "DrivableAreas", "DynamicObjects", "EgoVehicle", "Lanes",
                                 //"RoadSideParking", "RestArea", "SensorSectors"});
    ...
    {
        PlannerHNS::WayPoint p0 =
            PlannerHNS::WayPoint(pose_.position.x, pose_.position.y, pose_.position.z, tf::getYaw(pose_.orientation));
     ...
     */
    static std::vector<int> getTwoClosestIndices(const std::vector<PlannerHNS::WayPoint>& trajectory,
     ...
     */
    static double getSignedCurvature(const PlannerHNS::WayPoint& P1, const PlannerHNS::WayPoint& P2,
                                     const PlannerHNS::WayPoint& P3)
    {
        double curvature = 0;
        double cross_product =
            (P2.pos.x - P1.pos.x) * (P3.pos.y - P1.pos.y) - (P2.pos.y - P1.pos.y) * (P3.pos.x - P1.pos.x);
        if (cross_product != 0)
        {  // If the points are not collinear
            double area_triangle = (P1.pos.x * (P2.pos.y - P3.pos.y) + P2.pos.x * (P3.pos.y - P1.pos.y) +
                                    P3.pos.x * (P1.pos.y - P2.pos.y)) /
                                   2;
    ...
    {
        return sqrt((P2.position.x - P1.position.x) * (P2.position.x - P1.position.x) +
     ...
     * \Checks if the instantaneous test results hit the threshold value and updates the non-instantaneous test result
     * as pass and fail
     */
    ...
    /*!
     * \brief Check the size of thresholds, increments and decrements vector size from ROS parameter server : Called at
     ...
     */
    void performAction(const CRITICAL_LEVEL_& most_critical_level)
        ...
        switch (most_critical_level)
        {
            ...
            case ROAD_SIDE_PARKING:
                ...
                if (!is_road_side_parking_chosen_)
                {
                    ...
                    else  // if there is no valid parking we stop immediately (going one level up in term of
                          // criticality)
                    {
                        triggerSafetySwitch();
                ...
                {
                    if (isRoadSideParkingReachableBySSMP(road_side_parking_pose))  // if not we need to wait a bit to be
                                                                                   // able to park on the parking
                    {
    ...
    bool isThereAValidRoadSideParking()
    {
        ...
        for (grid_map::GridMapIterator it(gridmap_); !it.isPastEnd(); ++it)
        {
            if (gridmap_.at("RoadSideParking", *it) != 0)
            {
                ...
                if (isRoadSideParkingValid(distance_ego_to_parking, remaining_traj_length,
                                           getPerpendicularDistanceToGlobalPlanFrom(wp)))
                {
                    return true;
                }
            }
    ...
    static bool isRoadSideParkingValid(double distance_to_parking, double remaining_traj_length,
                                       double perpendicular_distance)
    {
        const double PERPENDICULAR_DISTANCE_THRESHOLD = 20;
        const double DISTANCE_TO_PARKING_LOW_THRESHOLD = 25;
        return (DISTANCE_TO_PARKING_LOW_THRESHOLD < distance_to_parking &&
                distance_to_parking < remaining_traj_length &&
                abs(perpendicular_distance) < PERPENDICULAR_DISTANCE_THRESHOLD);
    }
    ...
    geometry_msgs::PoseStamped findClosestRoadSideParking()
    {
        ...
        for (grid_map::GridMapIterator map_iterator(gridmap_); !map_iterator.isPastEnd(); ++map_iterator)
        {
            if (gridmap_.at("RoadSideParking", *map_iterator) != 0)
            {
                ...
                if (parking_positions_wp_by_id.find(gridmap_.at("RoadSideParking", *map_iterator)) !=
                    parking_positions_wp_by_id.end())
                    parking_positions_wp_by_id[gridmap_.at("RoadSideParking", *map_iterator)].push_back(wp);
                else
                    parking_positions_wp_by_id[gridmap_.at("RoadSideParking", *map_iterator)] =
                        std::vector<PlannerHNS::WayPoint>{ wp };
                ...
                double distance_ego_to_parking = PlannerHNS::PlanningHelpers::GetDistanceOnTrajectory_obsolete(
                    autoware_global_path_.front(), getEgoWaypointIndexOnGlobalPlan(), wp);
                if (isRoadSideParkingValid(distance_ego_to_parking, remaining_traj_length, info.perp_distance) &&
                    distance_ego_to_parking < min_distance)
                {
    ...
    double getRemainingTrajectoryLength()
    {
        ...
        double remaining_traj_length = PlannerHNS::PlanningHelpers::GetDistanceOnTrajectory_obsolete(
            autoware_global_path_.front(), pose_wp_index, autoware_global_path_.front().back());
    ...
    static bool isRestAreaValid(double distance_to_parking, double remaining_traj_length, double perpendicular_distance)
    {
        ...
        return (DISTANCE_ON_TRAJ_LOW_THRESHOLD < distance_to_parking && distance_to_parking < remaining_traj_length &&
                abs(perpendicular_distance) < PERP_DISTANCE_THRESHOLD);
    }
    ...
    {
        return PlannerHNS::PlanningHelpers::GetDistanceOnTrajectory_obsolete(autoware_global_path_.front(),
                                                                             getEgoWaypointIndexOnGlobalPlan(), wp);
    }
    ...
    bool isThereAValidRestArea()
    {
        ...
        for (grid_map::GridMapIterator map_iterator(gridmap_); !map_iterator.isPastEnd(); ++map_iterator)
        {
            ...
            {   //                std::cout << "distance_ego_rest_area: " << distance_ego_rest_area << "
                //                remaining_traj_length: " << remaining_traj_length << "   perp dist: " <<
                //                info.perp_distance << std::endl;
                if (isRestAreaValid(getRemainingDistanceOnGlobalPlanTo(wp), remaining_traj_length,
                                    getPerpendicularDistanceToGlobalPlanFrom(wp)))
                {
                    return true;
                }
        ...
        for (grid_map::GridMapIterator map_iterator(gridmap_); !map_iterator.isPastEnd(); ++map_iterator)
        {
            if (gridmap_.at("RestArea", *map_iterator) != 0)
            {
                ...
                if (rest_areas_wp_by_id.find(gridmap_.at("RestArea", *map_iterator)) != rest_areas_wp_by_id.end())
                    ...
                    rest_areas_wp_by_id[gridmap_.at("RestArea", *map_iterator)] =
                        std::vector<PlannerHNS::WayPoint>{ wp };
                int considered_wp_index = getClosestWaypointIndex(autoware_global_path_.front(), wp, 0);
                double distance_ego_rest_area = PlannerHNS::PlanningHelpers::GetDistanceOnTrajectory_obsolete(
                    autoware_global_path_.front(), getEgoWaypointIndexOnGlobalPlan(), wp);
                if (isRestAreaValid(distance_ego_rest_area, remaining_traj_length,
                                    getPerpendicularDistanceToGlobalPlanFrom(wp)) &&
                    distance_ego_rest_area < min_distance)
                {
  ...
  public:
     ...
     */
    explicit SafetySupervisor(ros::NodeHandle& nh)
      : nh_(nh)
      , var_switch_(SAFE)
      , tf_listener_(tf_buffer_)
      , current_fault_criticality_level_(CRITICAL_LEVEL_::INITIAL_GOAL)
    {
        ...
        pub_overwrite_trajectory_eval_ =
            nh_.advertise<autoware_msgs::LaneArray>("/adeye/overwrite_trajectory_eval", 1, true);
        ...
        sub_autoware_global_plan_ =
            nh.subscribe("/lane_waypoints_array", 1, &SafetySupervisor::autowareGlobalPlanCallback, this);
        sub_switch_request_ =
            nh.subscribe("safety_channel/switch_request", 1, &SafetySupervisor::switchRequestCallback, this);
