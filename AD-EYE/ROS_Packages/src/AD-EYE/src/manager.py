...
class ManagerStateMachine:
    ...
    current_state = (
        States.INITIALIZING_STATE
    )  # this is the current state of the state machine INITIALIZING_STATE
    ...
    def __init__(self):
        ...
        self.fault_manager = FaultInjectionManager(self)
    ...
    def activationRequestCallback(self, msg):
        if msg.data and self.current_state == self.States.ENABLED_STATE:
            ...
            not msg.data and self.current_state == self.States.ENGAGED_STATE
        ):  # deactivation
...
class Feature:
    ...
    def createFeatureControl(self):
        self.featureControl = FeatureControl(
            self.path, self.name, self.start_delay, self.stop_delay
        )
...
class ManagerFeaturesHandler:
    ...
    features["Fake_Localization"] = Feature(
        "Fake_Localization", "my_fake_localization.launch", 0, 0
    )
    features["Detection"] = Feature("Detection", "my_detection.launch", 0, 5)
    features["Mission_Planning"] = Feature(
        "Mission_Planning", "my_mission_planning.launch", 2, 5
    )
    features["Motion_Planning"] = Feature(
        "Motion_Planning", "my_motion_planning.launch", 0, 5
    )
    ...
    features["Experiment_specific_recording"] = Feature(
        "Experiment_specific_recording", "", 0, 0
    )
    ...
    def createLaunchPaths(self):
        ...
        for key in self.features:
            self.features[key].path = (
                adeye_package_path + launch_folder + self.features[key].path
            )

        self.features["Experiment_specific_recording"].path = (
            "/home/adeye/AD-EYE_Core/AD-EYE/ROS_Packages/src/AD-EYE"
...
class Manager:
    ...
    ROSBAG_PATH = (
        "/recording" + str(time.time()) + ".bag"
    )  # ~ is added as a prefix, name of the bag
    ROSBAG_COMMAND = (
        "rosbag record -a -O ~" + ROSBAG_PATH + " __name:=rosbag_recorder"
    )  # command to start the rosbag
    ...
    def __init__(self):
        ...
        rospy.Subscriber(
            "/Features_state", Int32MultiArray, self.featuresRequestCallback
        )
        rospy.Subscriber(
            "/switch_command", Int32, self.switchCallback
        )  # to check if safety channel is still alive
        self.state_pub = rospy.Publisher("manager/state", Int8, queue_size=1)  # for GUI
        self.features_pub = rospy.Publisher(
            "manager/features", Int32MultiArray, queue_size=1
        )  # for GUI
        self.switch_request_pub = rospy.Publisher(
            "/safety_channel/switch_request", Int32, queue_size=1
        )  # for GUI
    ...
    def runOnce(self):
        ...
        self.state_pub.publish(
            self.manager_state_machine.getState().value
        )  # publish the state machine state (for GUI)
            ...
            if msg.data[i] == 1:
                if self.manager_features_handler.features.keys()[i] in allowed_features:
                    message_features.append(
                        self.manager_features_handler.features.keys()[i]
                    )
    ...
    def checkSafetyChannel(self):
        if self.last_switch_time_initialized:
            if (
                rospy.Time.now() - self.last_switch_time
            ).to_sec() > self.SWITCH_TIME_THRESHOLD:
    ...
    def checkManagerState(self):
        ...
        if state != self.current_state:  # the state has changed since last iteration
            ...
            if state == self.manager_state_machine.States.INITIALIZING_STATE:
                ...
                msg = Int32()
                msg.data = 3
                self.switch_request_pub.publish(
                    msg
                )  # when we enter fault state we first force the switch to safety channel
    ...
    def isFeatureJustActivated(self, feature_name):
        return (
            feature_name in self.current_features
            and feature_name not in self.previous_features
        )
    ...
    def isFeatureJustDeactivated(self, feature_name):
        return (
            feature_name not in self.current_features
            and feature_name in self.previous_features
        )
    ...
    def startAndStopFeatures(self):
        for feature_name in self.manager_features_handler.features:
            if (
            ...
            ):  # recording needs special case as it is not a launch file like other features
                ...
                try:  # to not kill the manager when a launch is malformed (in that case an exception is thrown)
                    if self.isFeatureJustActivated(feature_name):
                        self.manager_features_handler.features[
                            feature_name
                        ].featureControl.start()
                    elif self.isFeatureJustDeactivated(feature_name):
                        self.manager_features_handler.features[
                            feature_name
                        ].featureControl.stop()
                    ...
                    rospy.logerr(
                        "Manager failed to start " + feature_name + ": " + str(exc)
                    )
